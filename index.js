...
app.post(
  '/users',
  [
    check('Username', 'Username requires minimum 5 characters').isLength({
      min: 5,
    }),
    check(
      'Username',
    ...
    Users.findOneAndRemove({ Username: req.params.Username })
      .then((user) => {
        if (!user) {
          ...
          res
            .status(200)
            .send(req.params.Username + ' account has been deleted!');
        }
      })
      .catch((err) => {
        console.error(err);
...
app.get(
  ...
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
    // BRING BACK AUTHENTICATION JWT TOKEN
    Movies.find()
      .then((movies) => {
